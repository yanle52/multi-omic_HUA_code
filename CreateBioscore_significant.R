...
library(survIDINRI)
#导入数据及合并数据集
HUAbio <- read.csv("/dssg/home/acct-sunkun/duxihao-1/UKB/HUA/HUAbiodataset.csv")
BioSign <- read.csv("/dssg/home/acct-sunkun/duxihao-1/UKB/HUA/Signatures/BioSign.csv")

diseases <- c("cad","stroke","hf","arrhyth","hyperten","t2d","ckd","gout","death")
HUAbio_cad <- subset(HUAbio,time_cad>0)
HUAbio_stroke <- subset(HUAbio,time_stroke>0)
HUAbio_hf <- subset(HUAbio,time_hf>0)
HUAbio_hyperten <- subset(HUAbio,time_hyperten>0)
HUAbio_arrhyth <- subset(HUAbio,time_arrhyth>0)
HUAbio_t2d <- subset(HUAbio,time_t2d>0)
HUAbio_ckd <- subset(HUAbio,time_ckd>0)
HUAbio_gout <- subset(HUAbio,time_gout>0)
HUAbio_death<- subset(HUAbio,time_death>0)
...
for (disease in diseases) {
  data_name <- paste0("HUAbio_", disease)
  current_data <- get(data_name)
  protein_cols <- BioSign %>%
  ...
  results <- foreach(i = 1:10, .packages = c("glmnet", "survival", "caret", "dplyr")) %dopar% {
    tryCatch({
      ...
      scores$Bioscore <- as.vector(predict(model, newx = test_x, type = "link"))
...
for (disease in diseases) {
  write.csv(all_disease_results[[disease]]$predictions, 
            paste0("/dssg/home/acct-sunkun/duxihao-1/UKB/HUA/Signatures/Biocore_", disease, ".csv"), 
            row.names = FALSE)
}
...
saveWorkbook(wb, file = "/dssg/home/acct-sunkun/duxihao-1/UKB/HUA/Signatures/Bioscore_beta.xlsx", overwrite = TRUE)
